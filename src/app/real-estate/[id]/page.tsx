...
import {
  ...
  Phone, Mail, Star, CheckCircle2, Calendar, Play,
  Building2, Car, Wifi, ShieldCheck, Dumbbell, Trees,
} from "lucide-react";
...
const mockReviews = [
  { name: "Adebayo Ogundimu", avatar: "https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=80&q=80", rating: 5, date: "1 month ago", comment: "The listing was accurate and the agent was very responsive. Viewing was arranged quickly and the property was exactly as described." },
  { name: "Funmilayo Adesanya", avatar: "https://images.unsplash.com/photo-1494790108377-be9c29b29330?w=80&q=80", rating: 4, date: "2 months ago", comment: "Great location and good value. The agent handled all paperwork professionally. Minor delay on documentation but overall a smooth experience." },
  { name: "Kingsley Nwachukwu", avatar: "https://images.unsplash.com/photo-1560250097-0b93528c311a?w=80&q=80", rating: 5, date: "3 months ago", comment: "Absolutely love the property. The neighbourhood is quiet and well-maintained. Would recommend this agency to anyone." },
];
...
export default function PropertyDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const handleContactSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    toast.success("Message sent to agent!", { description: "They will get back to you shortly." });
  ...
  return (
    <AppLayout>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {/* Image gallery */}
        <div className="mb-8">
          <div className="relative rounded-2xl overflow-hidden bg-secondary aspect-[16/9] sm:aspect-[2/1] group">
            <img
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <div className="bg-card border border-border rounded-2xl p-6">
              <div className="flex flex-wrap items-end justify-between gap-4 mb-4">
                <div>
                  <span className="font-display text-3xl font-bold text-primary">{property.price}</span>
                  {property.priceLabel && (
                    <span className="text-muted-foreground text-sm ml-2">{property.priceLabel}</span>
                  )}
                </div>
              </div>
              {(property.beds || property.baths || property.sqft || property.parkingSpaces || property.yearBuilt) && (
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 pt-4 border-t border-border">
                  ...
                  {property.parkingSpaces && (
                    <div className="text-center p-3 rounded-xl bg-secondary/50">
                      <Car strokeWidth={1} className="w-5 h-5 text-primary mx-auto mb-1" />
                      <p className="font-semibold text-foreground">{property.parkingSpaces}</p>
            ...
            <div className="bg-card border border-border rounded-2xl p-6">
              ...
              {property.yearBuilt && (
                <p className="text-sm text-muted-foreground mt-3">Year built: <span className="font-medium text-foreground">{property.yearBuilt}</span></p>
              )}
            </div>
                ...
                {property.amenities.map((amenity) => {
                  ...
                  return (
                    <div key={amenity} className="flex items-center gap-2.5 p-3 rounded-xl bg-secondary/50">
                      <Icon strokeWidth={1} className="w-4 h-4 text-primary shrink-0" />
            ...
            {property.videoUrl && (
              <div className="bg-card border border-border rounded-2xl p-6">
                <h2 className="font-display text-xl font-bold text-foreground mb-4">Video Tour</h2>
          ...
          <div className="lg:col-span-1 space-y-6">
            ...
            <div className="bg-card border border-border rounded-2xl p-5 sticky top-24">
              ...
              <div className="flex items-center gap-3 mb-5">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground flex items-center gap-1">
                    <CheckCircle2 strokeWidth={1.5} className="w-3.5 h-3.5 text-success" /> Verified Agent
                  </p>
                </div>
              </div>

              <div className="space-y-2 mb-5 text-sm">
                <a href={`tel:${property.agentPhone}`} className="flex items-center gap-2 text-muted-foreground hover:text-foreground transition-colors">
                  <Phone strokeWidth={1} className="w-4 h-4 text-primary" />
                  {property.agentPhone}
                </a>
                <a href={`mailto:${property.agentEmail}`} className="flex items-center gap-2 text-muted-foreground hover:text-foreground transition-colors">
              ...
              <div className="flex gap-2 mb-5">
                <Button className="flex-1 rounded-xl gap-1.5 text-sm" onClick={() => window.open(`tel:${property.agentPhone}`)}>
                  <Phone strokeWidth={1} className="w-4 h-4" /> Call
                </Button>
                <Button variant="outline" className="flex-1 rounded-xl gap-1.5 text-sm" onClick={() => window.open(`https://wa.me/${property.agentPhone.replace(/\D/g, "")}`)}>
              ...
              <div className="border-t border-border pt-4">
                <h4 className="font-semibold text-foreground text-sm mb-3 flex items-center gap-2">
                  <Calendar strokeWidth={1} className="w-4 h-4 text-primary" />
                  Send a Message
                </h4>
